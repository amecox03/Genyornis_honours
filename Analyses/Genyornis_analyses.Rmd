...
colours[which(data_plot$species == "newtoni")] <- "purple"
colours[which(data_plot$species == "lawsoni")] <- "green"
colours[which(data_plot$species == "woodburnei")] <- "black"
colours[which(data_plot$species == "maximus")] <- "red"
colours[which(data_plot$species == "modestus")] <- "blue"
colours[which(data_plot$genus == "Dinornis")] <- "orange"
colours[which(data_plot$genus == "Struthio")] <- "lightblue"
colours[which(data_plot$genus == "Pachyornis")] <- "brown"
...
#getting a model for predicting CIs
model_noD <- lm(fM.L ~ tbM.L, data=datanoD_plot)
summary(model)
...
#make a vector where any species name that has Genyornis is assigned purple
colours[which(largebirds_plot$species == "newtoni")] <- "purple"
colours[which(largebirds_plot$species == "lawsoni")] <- "green"
colours[which(largebirds_plot$species == "woodburnei")] <- "black"
colours[which(largebirds_plot$species == "maximus")] <- "red"
colours[which(largebirds_plot$species == "modestus")] <- "blue"
colours[which(largebirds_plot$genus == "Dinornis")] <- "orange"
colours[which(largebirds_plot$genus == "Struthio")] <- "lightblue"
colours[which(largebirds_plot$genus == "Pachyornis")] <- "brown"



#linear model of the femur width vs tibia width in large flightless birds
lb_widths <- lm(log(largebirds_data$fM.L)~log(largebirds_data$tbM.L))
...
#make dummy sequence of x for intervals. 
#REDUCING THE DATA SET TO 66
newxB <- seq(min_xB-1,max_xB+1,by = 0.05)

#plot confidence intervals

#This only works if the data are already logged. Not my preference but hey.
#THIS IS NOT WORKING BECAUSE THE newxB DATA SET ONLY HAS 66 VALUES, WHILE THE CI SET HAS 77
conf_intervalB <- predict(lb_widths, newdata=data.frame(tbM.L = newxB), interval="prediction", level = 0.95)


plot (largebirds_plot$fM.L ~ largebirds_plot$tbM.L, 
     pch=19, col=colours); abline(lb_widths, col="black" )


lines(newxB, conf_interval[,2], col="blue", lty=2)
lines(newxB, conf_interval[,3], col="blue", lty=2)
